...
do
    ...
    ((TestNumber+=1))
    time(valgrind --leak-check=full -q --error-exitcode=1 ./project <$file >tmp)
    echo
    ReturnCode=$?
    if [ $ReturnCode -ne 0 ]
    then
        echo VALGRIND wykrył błąd!
        echo Zwrócono kod wykonania valgrinda: $ReturnCode
        echo Treść wykonania valgrinda:
        echo
        gcc @opcje -g $1 -o project
        valgrind --leak-check=full -s --track-origins=yes --error-exitcode=1 ./project <$file
        echo
        echo Skrypt kończy działanie.
        rm project
        rm tmp
