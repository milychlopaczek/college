...
{
    for (SimpleQueueNode* node = queue->tail; node != NULL;) {
        SimpleQueueNode* next = node->next;
        free(node);
        node = next;
    }
...
Value SimpleQueue_pop(SimpleQueue* queue)
{
    ...
    // Find the first not taken item and free the nodes along the way (beware not to delete the tail).
    while (node != NULL && atomic_load(&node->item) == TAKEN_VALUE) {
        SimpleQueueNode* next = atomic_load(&node->next);
        if (next == NULL) { // We reached the tail, we can't delete it.
            atomic_store(&head->next, node);
    ...
    // Actualize the head after deleting the taken items.
    atomic_store(&head->next, node);
    if (node == NULL) {
        pthread_mutex_unlock(&queue->head_mtx);
        return EMPTY_VALUE;
    ...
    // If the queue will be empty after this pop, we mark the last item as taken instead of removing it.
    if (atomic_load(&node->next) == NULL) {
        atomic_store(&node->item, TAKEN_VALUE);
    } else {
